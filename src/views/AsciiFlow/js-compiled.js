...
function S(a) {
  ...
  for (var g = "", e = b.y;e <= c.y;e++) {
    ...
    g += l.replace(/\s+$/, "").replace(/\s[\u4E00-\u9FFF]+/g, (word) => word.slice(1)) + "\n";
  }
  return g;
}
function ua(a, b, c) {
...
Ga.prototype.r = function() {
  ...
  $(a).mousedown(function(a) {
    if(typeof window.siderWidth === 'undefined'){
      window.siderWidth = document.querySelector('.ant-layout-sider').clientWidth || 0
    }
    if (a.ctrlKey || a.metaKey) {
      var c = this.c;
      a = new p(a.clientX - window.siderWidth, a.clientY);
      c.mode = 1;
      c.G = a;
      c.H = c.view.offset;
    } else {
      c = this.c, a = new p(a.clientX - window.siderWidth, a.clientY), c.mode = 2, c.d.start(F(c.view, a));
    }
  }.bind(this));
  $(a).mouseup(function() {
    V(this.c);
  ...
  $(a).mousemove(function(a) {
    ...
    this.c.u(new p(a.clientX - window.siderWidth, a.clientY));
  }.bind(this));
};
function Y(a) {
  this.c = a;
